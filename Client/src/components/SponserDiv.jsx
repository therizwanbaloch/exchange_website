  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col items-center">
        ...
        <h2 className="text-[#1E3A8A] text-2xl sm:text-3xl font-bold mb-12 tracking-widest">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {sponsors.map((logo, index) => (
            <div
              key={index}
              className="bg-white/90 backdrop-blur-md border border-white/30 shadow-lg rounded-xl flex items-center justify-center p-4 hover:scale-105 transition-transform duration-300"
            >
              <img src={logo} alt={`Sponsor ${index + 1}`} className="max-h-12 object-contain" />
            </div>
          ))}
        </div>
      </div>
